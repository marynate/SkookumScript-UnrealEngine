...
// tracing, profiling, breakpoints, etc. can be added.
  ...
  // breakpoint) - see SkDebug::hook_expression()
...
enum eSkInvokeInfo
  ...
  // Locals only info
  SkInvokeInfo_temporaries    = 1 << 5,  // Include instance data members
  SkInvokeInfo_instance_data  = 1 << 6,  // Include instance data members
  SkInvokeInfo_class_data     = 1 << 7,  // Include class data members

  SkInvokeInfo__locals_def    = SkInvokeInfo_scope | SkInvokeInfo_temporaries | SkInvokeInfo_args | SkInvokeInfo_this | SkInvokeInfo_instance_data | SkInvokeInfo_class_data,

  // Callstack only info
  SkInvokeInfo_ignore_absent  = 1 << 8,  // If no call stack available do not print any info
  SkInvokeInfo_index          = 1 << 9,  // Include caller source character index
  SkInvokeInfo_updater        = 1 << 10, // Include the updater for invoked coroutines
  SkInvokeInfo_depth          = 1 << 11, // Include the call depth
...
class SkDebug
  ...
  // Common Methods
    ...
    // Debug Output
      ...
      static void    locals_string(AString * str_p, SkInvokedBase * invoked_p = nullptr, uint32_t flags = SkInvokeInfo__locals_def);
