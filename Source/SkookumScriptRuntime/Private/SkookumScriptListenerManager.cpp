...
void SkookumScriptListenerManager::grow_inactive_list(uint32_t pool_incr)
  ...
  for (uint32_t i = 0; i < pool_incr; ++i)
    {
    m_inactive_list.append(*ConstructObject<USkookumScriptListener>(USkookumScriptListener::StaticClass(), (UObject*)GetTransientPackage(), NAME_None, RF_RootSet)); // RF_RootSet so it won't be garbage collected
    }
